...
def test_archive_excludes_local_credentials_and_reports(tmp_path):
    ...
    with ZipFile(output) as archive:
        assert all(name.startswith("matchtrader-python/v3/") for name in archive.namelist())
        recorded = json.loads(archive.read("matchtrader-python/v3/HANDOFF-MANIFEST.json"))
        for row in recorded:
            content = archive.read("matchtrader-python/v3/" + row["path"])
