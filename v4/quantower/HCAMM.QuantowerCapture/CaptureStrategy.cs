...
public sealed class CaptureStrategy : Strategy
{
    ...
    protected override void OnRun()
    {
        ...
        try
        {
            ...
            outbox.Acknowledged = (item, status, reason, brokerId) => {
                if (item.Kind is "ACCEPTED" or "REJECTED")
                    Log(EventLog.Result(item, status, reason, brokerId));
            };
            ...
            outbox.Start();
            Log("v4 native capture attached. P01 manual labels and per-event logging enabled. Copying is controlled by the dashboard.");
        }
        catch { OnStop(); throw; }
    }
    ...
    void Capture(CaptureEvent item)
    {
        try {
            outbox?.Enqueue(item);
            if (item.Kind is "REQUEST" or "ACCEPTED" or "REJECTED" or "FILL" or "ACCOUNT") Log(EventLog.Captured(item));
        }
