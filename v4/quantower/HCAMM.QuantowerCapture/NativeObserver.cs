...
public sealed class NativeObserver(CaptureConfig config, Action<CaptureEvent> send, Action? fault = null) : IDisposable
{
    ...
    CaptureEvent? FromRequest(RequestParameters request)
    {
        ...
        return item == null ? null : item with { RequestId = run + ":" + request.RequestId, SendingSource = request.SendingSource ?? "", Source = config.Attribute(request.SendingSource, item.SourceLabel) };
    }
