...
Check(config.Attribute(null, "P01RR_123456_2") == "P01" && config.Attribute(null, "M:mirror#1") == "UNKNOWN" && config.Attribute(null, "") == "UNKNOWN", "P01 manual labels exclude mirrors and blank sources");
Check(config.Attribute("HCAMM:R01", "P01RR_123456_2") == "R01", "Named strategy attribution takes precedence over comment");
...
Check(EventLog.Captured(item with { SourceLabel = "P01RR_123456_2", Sl = 1.1498m }).Contains("SL=1.1498") && EventLog.Result(item, "accepted", "line\nbreak", "aqua1").Contains("broker=aqua1") && !EventLog.Result(item, "held", "line\nbreak", "").Contains('\n'), "EventLog distinguishes broker acknowledgement and removes control characters");
...
using (var outbox = new DurableOutbox(config, new FakeHandler(async request => {
    ...
    outbox.Acknowledged = (e, status, reason, brokerId) => { if (e.EventId == item.EventId) acknowledgements++; };
    ...
    Check(acknowledgements == 1, "Only acknowledged events are logged as delivery results");
}
