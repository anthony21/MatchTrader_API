...
class SymbolMapping(BaseModel):
    ...
    @classmethod
    def drop_legacy_min_box(cls, value):
        # The minimum box moved onto the copy configuration (with its own switch), so it is no
        # longer part of the symbol map. Files and forms that still carry a min_box key load
        # unchanged; the key is dropped rather than rejected.
        if isinstance(value, dict) and "min_box" in value:
            value = {k: v for k, v in value.items() if k != "min_box"}
        return value
