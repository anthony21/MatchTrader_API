...
class DashboardController:
    ...
    def _observe_ledger_row(self, path, offset, payload):
        """One fresh R01 ledger row: the R01 lane decides on it, then the journal records the
        observation. The CSV tail is a real-time feed (each row is read as it is appended), so it
        drives copying directly. With an account selected the observation goes to the shadow journal
        (the bridge page's LEDGER rows); without one it goes to the in-memory feed."""
        row = payload['record']
