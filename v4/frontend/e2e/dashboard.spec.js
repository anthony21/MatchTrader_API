...
test('Copy settings offers a single forwarding switch and broker Live or Paper', async ({ page, request }) => {
  await page.goto(url + '/?page=settings')
  const section = page.getByRole('region', { name: 'TradingBox forwarding' })
  const toggle = section.getByRole('switch', { name: 'Forwarding', exact: true })
  await expect(toggle).toHaveText('Forwarding Off')
  await expect(page.getByRole('switch', { name: 'Live or Paper' })).toHaveText('Paper')
  ...
  const off = await request.post(url + '/api/hcamm/events', { headers: { 'X-HCAMM-Key': 'test-tradingbox-native-key' }, data: { kind: 'X17_SETUP', exactPrice: '4300.00' } })
  expect(off.status()).toBe(202)
  expect((await off.json()).forwarded).toBe(false)
  await toggle.click()
  await expect(toggle).toHaveText('Forwarding On')
  // No signals are submitted while on; upstream delivery uses isolated unit fixtures.
  await toggle.click()
  await expect(toggle).toHaveText('Forwarding Off')
  await page.setViewportSize({ width: 390, height: 844 })
  expect(await page.evaluate(() => [...document.querySelectorAll('main *')].filter(e => e.getBoundingClientRect().right > innerWidth).map(e => e.tagName + '.' + e.className))).toEqual([])
