...
test('shows actual position identity and pnl, distinguishes empty and unavailable', async () => {
  ...
  await wrapper.setProps({ positions: [{ id: 'p1', profit: '-2.50', netProfit: '0' }] })
  expect(wrapper.findAll('tbody td')[7].text()).toBe('0')
  await wrapper.setProps({ positions: [{ id: 'p1', profit: '-2.50' }] })
  expect(wrapper.findAll('tbody td')[7].text()).toBe('-2.50')
  await wrapper.setProps({ positions: [{ id: 'p1' }] })
  expect(wrapper.findAll('tbody td')[7].text()).toBe('—')
